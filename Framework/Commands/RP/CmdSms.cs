    ...
    public class CmdSms : IRocketCommand
    {
        ...
        public void Execute(IRocketPlayer caller, string[] args)
        {
            ...
            if ((object)target != null) 
            {
                var message = args;
                message[0] = "";
                if (string.Join(" ", message).Length < 2) return;

                EffectManager.sendUIEffect(5071, 1751, player.TransportConnection, false);
                ChatManager.serverSendMessage($"<color=#2196F3><b>SMS</b> | pre {target.Name} : </color><color=#ffffff>{string.Join(" ", message)}</color>", Color.white, null, player.Player.channel.owner, EChatMode.SAY, "https://i.ibb.co/g6s9sYj/sms.png", true);
                
                ChatManager.serverSendMessage($"<color=#2196F3><b>SMS</b> | od {player.Name} : </color><color=#ffffff>{string.Join(" ", message)}</color>", Color.white, null, target.Player.channel.owner, EChatMode.SAY, "https://i.ibb.co/g6s9sYj/sms.png", true);
